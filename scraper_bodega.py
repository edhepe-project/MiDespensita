...
def run():
    ...
    while terminos_pendientes:
        ...
        try:
            ...
            reiniciar = False  # bandera para romper y abrir nuevo navegador

            while terminos_pendientes and not reiniciar:
                termino = terminos_pendientes[0]
                print(f"Buscando en Bodega: {termino} (Intento {intentos_actuales+1})...")

                url = f"https://www.bodegaaurrera.com.mx/search?q={termino}"
                driver.execute_script(f"window.location.href = '{url}';")

                # Esperar y revisar si cayó en CAPTCHA
                time.sleep(4)
                ...
                # Verificar si el CAPTCHA persiste tras el intento de resolución
